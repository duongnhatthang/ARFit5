...
//    private final String OPEN_WEATHER_URL = "http://api.openweathermap.org/data/2.5/weather?q=%s&mode=json";
    private final String OPEN_WEATHER_URL = "http://api.openweathermap.org/data/2.5/weather?q=%s&appid=2de143494c0b295cca9337e1e96b00e0&mode=json";
    private EditText mEdtLocation, mEdtCountry, mEdtTemperature, mEdtHumidity, mEdtPressure;
    private Button mBtnLoadWeather;
    private HandleJSON mObjectHandleXML;
        ...
        mBtnLoadWeather.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (NetworkConnection.isNetworkAvailable(getApplicationContext())) {
                    ...
                    new DownloadWeatherContent().executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, contentURL);
                } else {
    ...
    private class DownloadWeatherContent extends AsyncTask<String, Void, HandleJSON> {
        @Override
        protected HandleJSON doInBackground(String... params) {
            mObjectHandleXML = new HandleJSON(params[0]);
            mObjectHandleXML.fetchXML();
            return mObjectHandleXML;
        }

        @Override
        protected void onPostExecute(HandleJSON handleJson) {
            super.onPostExecute(handleJson);
